...
The pipeline validates itself at three points. Every check is a query in the
notebook — you can see the result, not just a claim.

For *why* each check exists, see §7 of `00_documentation`.

### What to expect

| Notebook section | Check | Expected result |
|---|---|---|
| 1.2 | Row counts, 11 tables | all non-zero |
| 2.1 | Primary key integrity | `PASS` on every row |
| 2.4 | Track price distribution | exactly 2 values: `0.99`, `1.99` |
| 2.6 | InvoiceDate time component | 0 |
| 2.7 | Referential integrity, 9 FKs | 0 orphans |
| 3.1 | `dq_status` breakdown | 0 `REJECT` |
| 3.1 | Raw → clean row counts | match exactly |
| 3.1 | Invoice total vs `SUM(line_amount)` | **0 mismatches** |
| 3.1 | Date coverage | 60 months, no gaps |
| 4.2 | Fact grain | rows = distinct keys |
| 4.2 | Clean revenue = mart revenue | both **$2,328.60** |
| 4.2 | Fact FK orphans | 0 |
| 4.2 | Dimension key uniqueness | rows = keys, all 4 dims |
| 4.2 | Revenue after full star join | **$2,328.60** |

### Stop conditions

Two failures mean stop rather than continue:

- **2.1 reports `FAIL`** — a duplicate dimension key will multiply revenue in the
  fact table.
- **4.2 clean revenue ≠ mart revenue** — a join in the fact build is dropping rows.

Everything else can be investigated after the run.
...
| Date span | 2021-01-01 → 2025-12-22 (60 months, no gaps) |
| Tracks in catalogue | 3,503 |
| Rows recovered from the CSV fix | 6 |
| FK orphans found | 0 |
